          ...
          <section className="max-w-theme flex w-full flex-col items-start justify-start gap-10 px-5">
            {product.images && (
              <section className="flex w-full flex-col items-center justify-center gap-5 border border-zinc-200 bg-zinc-100 p-5 lg:p-5">
                ...
                <section className="relative aspect-video max-h-[450px] w-full overflow-hidden">
                  {product.images.map((img, key) => (
                    <img
                      src={img}
                      alt={postData.title}
                      key={`${postData.slug}-active-image-${key}`}
                      className={`${key.toString() === activeImage ? 'pointer-events-auto visible opacity-100' : 'pointer-events-none invisible opacity-0'} absolute aspect-video max-h-[450px] w-full border border-zinc-200 object-cover object-center shadow-xl transition-all duration-500`}
                    />
                  ))}
                </section>
